...
export default function ShippingPage() {
  ...
  const [htmlElementsData, setHtmlElementsData] = useState(() => {
    function getLabelLatLng(code) {
      switch (code) {
        ...
        case 'ES': return { lat: 40.4637 - 6, lng: -3.7492 };
        case 'MY': return { lat: 4.2105 - 6, lng: 101.9758 };
        case 'KE': return { lat: -0.0236 - 6, lng: 37.9062 };
        case 'SG': return { lat: 1.3521 - 6, lng: 103.8198 };
    ...
    fetch('/countries.geojson')
      ...
      .catch(error => {
        ...
        fetch('https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson')
          .then(res => res.json())
          .then(data => {
            const targetCountries = ['Spain', 'Malaysia', 'Kenya', 'Singapore', 'Indonesia'];
  ...
  const getCountryColor = (countryData) => {
    ...
    if (selectedCountry) {
      if ((selectedCountry === 'ES' && (iso === 'ES' || name === 'Spain')) ||
    ...
    if (selectedCountry === countryCode) {
      ...
      if (globeEl.current && isGlobeReady) {
        globeEl.current.pointOfView({
          ...
          lng: indonesiaCoords.lng,
          altitude: GLOBE_ALTITUDE
        }, 1000); // Durasi sama dengan animasi panel (1000ms)
      }
        ...
        >
          <div className="p-6">
            ...
            <div className="mb-6">
              ...
              >
                Alur pengiriman dan estimasi biaya ke {countries.find(c => c.code === selectedCountry)?.name}
              </p>
            </div>
            ...
            <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
              ...
              >
                ⚠️ Estimasi biaya dapat berubah tergantung jenis barang, berat, dan kondisi pasar saat ini.
              </p>
            </div>
          </div>
        </div>
